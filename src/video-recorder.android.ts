...
class SurfaceTextureListener extends java.lang.Object implements android.view.TextureView.SurfaceTextureListener {
    ...
    public onSurfaceTextureAvailable(surface, width, height) {
        ...
        if (this.owner) {
            this.owner.notify({ eventName: VideoViewCommon.availableEvent, object: this.owner });
        }
    }

    public onSurfaceTextureSizeChanged(surface, width, height) {
        console.log('Sureface Texture Changed : width : ' + width + ' height: ' + height);
    }
...
export class VideoRecording {
    ...
    constructor({ videoView, cameraId, file, profile, outputFormat, thumbnailCount }) {
    ...
    private prepareMediaRecorder() {
        try {
            ...
            this.mediaRecorder.setAudioSource(android.media.MediaRecorder.AudioSource.DEFAULT);
    ...
    public startPreview() {
        ...
        if (supportedFocusModes.contains(android.hardware.Camera.Parameters.FOCUS_MODE_AUTO)) {
        ...
        if (supportedSceneModes.contains(android.hardware.Camera.Parameters.SCENE_MODE_NIGHT)) {
        ...
        if (previewFpsRange[0] === previewFpsRange[1]) {
            ...
            for (let i = 0; i < supportedFpsRanges.size(); i++) {
                ...
                if (range[0] !== range[1]) {
                    console.log('Setting preview fps range to ' + range[0] + ' - ' + range[1]);
    ...
    extractThumbnails() {
        ...
        let METADATA_KEY_DURATION = mediaMetadataRetriever
        ...
        for (let index = 0; index < this.thumbnailCount; index++) {
            let bmpOriginal = mediaMetadataRetriever.getFrameAtTime((index * it) * 1000, MediaMetadataRetriever.OPTION_CLOSEST);
            ...
            let bmpScaledSize = android.graphics.Bitmap.createScaledBitmap(bmpOriginal, bmpOriginal.getWidth(), bmpOriginal.getHeight(), false);
