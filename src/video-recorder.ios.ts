import { Common as VideoViewCommon, RECORDING_STATE } from "./video-recorder.common";
...
export class VideoRecorder {
    ...
    public static createRecording(options: VideoRecordingOptions): VideoRecording {
...
export class VideoRecording {
    ...
    public startPreview() {
        ...
        try {
            ...
            let cameraDeviceInput = AVCaptureDeviceInput.deviceInputWithDeviceError(this.camera);
            ...
            let audioDeviceInput = AVCaptureDeviceInput.deviceInputWithDeviceError(audioDevice);
            ...
            let previewLayer = AVCaptureVideoPreviewLayer.layerWithSession(this.captureSession);

            if (previewLayer) {
                previewLayer.bounds = CGRectMake(0, 0, this.videoView.bounds.size.width, this.videoView.bounds.size.height);
                previewLayer.position = CGPointMake(this.videoView.bounds.size.width / 2, this.videoView.bounds.size.height / 2);
    ...
    start() {
        this
            .movieFileOutput
            .startRecordingToOutputFileURLRecordingDelegate(NSURL.URLWithString("file://" + this.file), new CaptureDelegate());
    ...
    public switchCamera() {
        ...
        for (let i = 0; i < inputs.count; i++) {
            ...
            if (input.isKindOfClass(<any>(AVCaptureDeviceInput)) && (<AVCaptureDeviceInput>input).device.hasMediaType(AVMediaTypeVideo)) {
                break;
            }
        }
        ...
        let newCamera = CameraHelper
            .getCamera(input.device.position === AVCaptureDevicePosition.Back ? AVCaptureDevicePosition.Front : AVCaptureDevicePosition.Back);

        let newVideoInput: AVCaptureDeviceInput;

        try {
            newVideoInput = AVCaptureDeviceInput.deviceInputWithDeviceError(newCamera);
    ...
    extractThumbnails() {        
        this.thumbnails = [];
        let asset = AVURLAsset.alloc().initWithURLOptions(NSURL.URLWithString("file://" + this.file), null);
        ...
        for (let index = 0; index < this.thumbnailCount; index++) {
            ...
            let outputFilePath = this.file.substr(0, this.file.lastIndexOf('.')) + '_thumbnail_' + index + '.png';
            let ok = UIImagePNGRepresentation(image).writeToFileAtomically(outputFilePath, true);
...
class CaptureDelegate extends NSObject implements AVCaptureFileOutputRecordingDelegate {
...
class CameraHelper {
    public static getCamera(cameraId: AVCaptureDevicePosition) {
        let discoverySession = AVCaptureDeviceDiscoverySession
            .discoverySessionWithDeviceTypesMediaTypePosition(NSArray.arrayWithObject(AVCaptureDeviceTypeBuiltInWideAngleCamera), AVMediaTypeVideo, AVCaptureDevicePosition.Unspecified);
